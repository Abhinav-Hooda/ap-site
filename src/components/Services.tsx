...
const services = [
  {
    ...
    icon: (
      <svg className="size-12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        <div className="mt-20 grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service, idx) => (
            ...
            >
              <div className="mb-8 flex size-24 items-center justify-center rounded-2xl bg-muted text-foreground transition-all duration-300 group-hover:bg-accent group-hover:text-white group-hover:scale-110 group-hover:shadow-xl group-hover:shadow-accent/20">
                {service.icon}
              </div>
              <h3 className="font-mulish text-xl font-bold text-foreground">
                {service.title}
              </h3>
              <p className="font-inter mt-4 text-muted-foreground leading-relaxed">
                {service.description}
              </p>
            </Motion.div>
          ))}
